...
const ItemsForCategoryPage = (props) => {
    ...
    useEffect(() => {
        const updateBrands = () => {
            ...
            items.forEach((item) => {
                if(item.name.toLowerCase().includes(props.searchQuery.toLowerCase())){
    ...
    return (
        <div>
            <TextBanner text={"Список товаров по запросу '"+props.searchQuery+"':"}/>
            <div className={"Items"}>
                <MySelect
                    value={selectedSort}
                    onChange={sortItems}
                    ...
                    {isLoading ? (
                        ...
                        : items.length ? (
                            items.map((item, index) => {
                                if (selectedBrand === "Выбрать бренд" && item.name.toLowerCase().includes(props.searchQuery.toLowerCase())){
                                    if (itemsExist === false){
                                        setItemsExist(true)
                                    }
                                    return Item(item)
                                }else if(selectedBrand === item["brand"].toLowerCase() && item.name.toLowerCase().includes(props.searchQuery.toLowerCase())){
                                    if (itemsExist === false){
                                        setItemsExist(true)
                                    }
                                    return Item(item)
